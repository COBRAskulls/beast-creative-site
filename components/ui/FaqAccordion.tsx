      ...
      {faqs.map((faq, i) => (
        <div key={faq.question}>
          ...
          >
            ...
            <span className={`flex-shrink-0 w-7 h-7 rounded-full border-2 border-light-border flex items-center justify-center transition-all duration-200 ${open === i ? "bg-beast-pink border-beast-pink rotate-45" : "group-hover:border-beast-pink"}`}>
          ...
          >
            <p className="text-[#717171] text-sm leading-relaxed">{faq.answer}</p>
          </div>
        </div>
      ))}
    </div>
